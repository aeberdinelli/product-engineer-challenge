...
var handler = async (event) => {
  try {
    ...
    const { psychiatristId, date, startTime } = body;
    let scheduleBlocks;
    try {
      ({ scheduleBlocks } = await getPsychiatristSchedule(psychiatristId, date));
    } catch (err) {
      if (err instanceof Error && err.message === "NOT_FOUND") {
        return { statusCode: 404, body: JSON.stringify({ error: "Psychiatrist not found" }) };
      }
      throw err;
    }
    ...
    const requestedEnd = requestedStart + APPOINTMENT_MINUTES;
    let isAvailable = false;
    for (const block of scheduleBlocks) {
      if (requestedStart >= block.start && requestedEnd <= block.end) {
        const overlaps = bookedIntervals.some(
          (booked) => !(requestedEnd <= booked.start || requestedStart >= booked.end)
        );
        if (!overlaps) {
          isAvailable = true;
        }
    ...
    if (!isAvailable) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Requested time is not available" })
      };
    }
        ...
        Item: (0, import_util_dynamodb2.marshall)(
          {
            ...
            startTime,
            endTime: `${String(Math.floor(requestedEnd / 60)).padStart(2, "0")}:${String(
              requestedEnd % 60
            ).padStart(2, "0")}`,
            psychiatristId,
            date
