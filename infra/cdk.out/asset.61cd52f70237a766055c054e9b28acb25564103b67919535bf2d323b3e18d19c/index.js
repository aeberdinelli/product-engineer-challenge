...
var handler = async (event) => {
  try {
    ...
    const profile = (0, import_util_dynamodb.unmarshall)(profileResponse.Item);
    ...
    const typesToProcess = requestedType === "ALL" ? ["ONLINE", "IN_PERSON"] : [requestedType];
    ...
    for (const type of typesToProcess) {
      ...
      const blocks = schedules.filter((scheduleItem) => scheduleItem.day === weekday).map((scheduleItem) => {
        return {
    ...
    if (scheduleByType.ONLINE.length === 0 && scheduleByType.IN_PERSON.length === 0) {
      return {
        statusCode: 200,
        body: JSON.stringify([])
      };
    ...
    for (const type of typesToProcess) {
      ...
      const bookedResponse = await client.send(
        new import_client_dynamodb.QueryCommand({
          TableName: process.env.TABLE_NAME,
          IndexName: "GSI2",
          KeyConditionExpression: "GSI2PK = :pk AND begins_with(GSI2SK, :psychiatristPrefix)",
      ...
      const intervals = (bookedResponse.Items || []).map((raw) => {
