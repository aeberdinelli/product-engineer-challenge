...
var fromUtcIsoToZoneHHmm = (utcIso, zone) => {
...
var handler = async (event) => {
  try {
    ...
    const userZone = event.queryStringParameters?.tz;
    ...
    let scheduleBlocks;
    try {
      const item = await getPsychiatristSchedule(psychiatristId, date);
      profile = item.profile;
      scheduleBlocks = item.scheduleBlocks;
    } catch (err) {
    ...
    if (!profile?.timezone) {
      return {
        ...
        body: JSON.stringify({ error: "Psychiatrist timezone not set" })
      };
    }
    ...
    for (const bookedInterval of bookedIntervals) {
      ...
      for (const openInterval of freeIntervals) {
        const noOverlap = bookedInterval.end <= openInterval.start || bookedInterval.start >= openInterval.end;
        if (noOverlap) {
    ...
    for (const { start, end } of freeIntervals) {
      for (let currentMinute = start; currentMinute + CYCLE_MINUTES <= end; currentMinute += CYCLE_MINUTES) {
        const startHH = String(Math.floor(currentMinute / 60)).padStart(2, "0");
        const startMM = String(currentMinute % 60).padStart(2, "0");
        const endHH = String(Math.floor((currentMinute + APPOINTMENT_MINUTES) / 60)).padStart(2, "0");
        const endMM = String((currentMinute + APPOINTMENT_MINUTES) % 60).padStart(2, "0");
        const startLocal = `${startHH}:${startMM}`;
        const endLocal = `${endHH}:${endMM}`;
        const startUtc = toUtcIso(date, startLocal, profile.timezone);
        const endUtc = toUtcIso(date, endLocal, profile.timezone);
        const slot = {
          startUtc,
          endUtc
        };
        if (userZone && userZone.trim().length > 0) {
          slot.displayStart = fromUtcIsoToZoneHHmm(startUtc, userZone);
          slot.displayEnd = fromUtcIsoToZoneHHmm(endUtc, userZone);
        }
        slots.push(slot);
      }
    }
